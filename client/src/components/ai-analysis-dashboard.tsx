...
export default function AIAnalysisDashboard({ submissionId, examTitle }: AIAnalysisDashboardProps) {
  ...
  useEffect(() => {
    if (storedReports && storedReports.length > 0 && !generatedReport) {
      setGeneratedReport(storedReports[0].content);
    }
                ...
                {storedReports && storedReports.length > 0 && (
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500">
                      Generated: {new Date(storedReports[0].generatedAt).toLocaleString()}
                    </div>
                    <div className="text-xs text-gray-500">
                      Violations: {storedReports[0].violationCount} | Suspicion: {storedReports[0].suspicionLevel}%
                    </div>
                  </div>
                )}
              </div>
              ...
              {generateReportMutation.isPending ? (
                  ...
                  {storedReports && storedReports.length > 0 && (
                    <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-3">
                      <div className="flex items-center gap-2 text-green-700 dark:text-green-300 text-sm">
                        ...
                        <span>Report retrieved from database - no AI tokens used</span>
                      </div>
                    </div>
                  )}
                ...
                <div className="text-center py-8">
                  ...
                  <Button onClick={() => generateReportMutation.mutate()}>
                    <FileText className="h-4 w-4 mr-2" />
                    {storedReports && storedReports.length > 0 ? 'Load Stored Report' : 'Generate Report'}
                  </Button>
                </div>
              )}
            </CardContent>
